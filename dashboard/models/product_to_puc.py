        ...
        uberpuc_update_sql = """
                            UPDATE
                                dashboard_producttopuc ptp
                                    LEFT JOIN
                                factotum_prod.dashboard_producttopucclassificationmethod cm ON cm.id = ptp.classification_method_id
                                    LEFT JOIN
                                (SELECT 
                                ...
                                FROM
                                    (dashboard_producttopuc ptp
                                LEFT JOIN dashboard_producttopucclassificationmethod cm ON cm.id = ptp.classification_method_id)) ptp_rank ON ((ptp.product_id = ptp_rank.product_id
                                    AND cm.rank > ptp_rank.rank))
